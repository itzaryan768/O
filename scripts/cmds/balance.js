...
onStart: async function ({ api, args, message, event, threadsData, usersData, dashBoardData }) {
   ...
   // Define currentDate and currentTime variables outside of the if block
    ...
    if (command === "transfer") {
      ...
      if (userMoney < amount) {
        message.reply("⛔ You don't have enough money to transfer.");
        return;
      }
      ...
      if (recipientData) {
        ...
        if (senderMoney >= amount) {
          ...
          message.reply(`⛔ You don't have enough money to transfer, please check your balance then try again your request. (◕ᴗ◕✿)\n💰 𝗖𝘂𝗿𝗿𝗲𝗻𝘁 𝗕𝗮𝗹𝗮𝗻𝗰𝗲\n➤ ${userMoney}\n🆔 𝗜𝗗\n➤ ${senderID}\n📅 𝗗𝗮𝘁𝗲\n➤ ${currentDate}\n⏰ 𝗧𝗶𝗺𝗲\n➤ ${currentTime}\n✤━━━━━━━━━━━━━✤`);
        }
      } else {
        message.reply("⛔ Recipient not found. (◍•ᴗ•◍)");
      }
